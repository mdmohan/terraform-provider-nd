...
func NewInventoryFSM(
	...
	inv := &InventoryFSM{
		...
		dg:           dg,
		deadline:     time.Now().Add(MaxFSMTimeout),
	}
	...
	inv.deadline = time.Now().Add(time.Duration(len(inv.switchesData.Switches)) * MaxFSMTimeout)
	return inv
}
...
func (inv *InventoryFSM) checkDeadline(ctx context.Context, e *fsm.Event) bool {
	...
	if time.Now().After(inv.deadline) {
		inv.triggerFail(ctx, e, "timeout in state %s after %v", e.Dst, MaxFSMTimeout)
		return false
	}
	return true
}
