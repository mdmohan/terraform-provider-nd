...
func (r *vpcPairResource) rscCreateVpcPair(ctx context.Context, dg *diag.Diagnostics, input *VpcPairModel) {
	...
	log.Printf("Creating vPC pair for fabric %s switch %s", inData.FabricName, inData.SwitchId)

	vpcPairAPI := api.NewVpcPairAPI(nil, r.manageClient.ApiClient)
	vpcPairAPI.FabricName = inData.FabricName
	vpcPairAPI.SwitchID = inData.SwitchId
...
func (r *vpcPairResource) rscGetVpcPair(ctx context.Context, dg *diag.Diagnostics, in *VpcPairModel) {
	...
	vpcPairAPI.SwitchID = in.SwitchId.ValueString()
...
func (r *vpcPairResource) rscUpdateVpcPair(ctx context.Context, dg *diag.Diagnostics, vpcPairModel *VpcPairModel) {
	...
	log.Printf("Updating vPC pair for fabric %s switch %s", inData.FabricName, inData.SwitchId)
	...
	log.Printf("Updated vPC pair for fabric %s switch %s", inData.FabricName, inData.SwitchId)
}
...
func (r *vpcPairResource) rscDeleteVpcPair(ctx context.Context, dg *diag.Diagnostics, state *VpcPairModel) {
	...
	log.Printf("Deleting vPC pair for fabric %s switch %s", inData.FabricName, inData.SwitchId)
...
func mergeVpcPairIdentity(outData *NDFCVpcPairModel, fallback *NDFCVpcPairModel) {
	if outData == nil || fallback == nil {
		return
	}

	if outData.FabricName == "" {
		outData.FabricName = fallback.FabricName
	}
	if outData.PeerSwitchId == "" {
		outData.PeerSwitchId = fallback.PeerSwitchId
	}
	if outData.SwitchId == "" {
		outData.SwitchId = fallback.SwitchId
	}
	outData.Deploy = fallback.Deploy
	if outData.UseVirtualPeerlink == nil {
		outData.UseVirtualPeerlink = fallback.UseVirtualPeerlink
	}
}
...
func setVpcPairID(model *VpcPairModel) {
	...
	if model.FabricName.IsNull() || model.FabricName.IsUnknown() || model.SwitchId.IsNull() || model.SwitchId.IsUnknown() {
		return
	}

	model.Id = types.StringValue(fmt.Sprintf("%s/%s", model.FabricName.ValueString(), model.SwitchId.ValueString()))
}
